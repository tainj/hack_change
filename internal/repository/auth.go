...
func (r *AuthUserRepository) Register(ctx context.Context, user *models.User) error {
	...
	query := sq.Insert("users").
		Columns("id", "first_name", "last_name", "middle_name", "email", "password_hash", "role").
		Values(user.ID, user.FirstName, user.LastName, user.MiddleName, user.Email, user.PasswordHash, string(user.Role)).
...
func (r *AuthUserRepository) GetByEmail(ctx context.Context, email string) (*models.User, error) {
	...
	query := sq.Select("id", "first_name", "last_name", "middle_name", "email", "password_hash", "role", "created_at", "updated_at").
		From("users").
